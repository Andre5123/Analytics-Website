...
const menuId = parseInt(menuHeader.dataset.menuId)
...
newItemButton.addEventListener("click", ()=>{
    ...
    itemsContainer.insertBefore(newItem, itemTemplate.nextElementSibling);
    ...
    .then(data => {
        if (data.success) {
            if (data.itemId) {
                ...
                itemTemplate.nextElementSibling.dataset.itemId = data.itemId;
                console.log("Item id successfully created in server")
            }
...
itemsContainer.addEventListener("input", (event)=>{
    let deal = event.target.closest(".deal")
    if (deal) {
